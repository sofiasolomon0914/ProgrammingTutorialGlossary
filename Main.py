...
my_dictionary = {
    ...
    'tuple': 'An immutable sequence of elements.', 'tuple assignment': 'An assignment with a sequence on the right '
    'side and a tuple of variable on the left. The right side is evaluated and then its elements are assigned to the '
    'variables on the left.', 'brittle code': 'Code that works when the input data is in a particular format but is '
    ...
    'an example, web traffic usually uses port 80 while email uses port 25.'
}
